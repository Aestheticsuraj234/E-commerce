      ...
      <section className="bg-gradient-to-r min-h-screen from-indigo-200 via-red-200 to-lime-100">
        <motion.div   variants={staggerContainer}
          ...
          <h1 className="font-extrabold py-12 text-gray-800 text-center text-xl">
            ...
            {Object.keys(products).length === 0 && <h1 className="font-bold text-center text-4xl ">Sorry All the Gems Products are Out of Stock</h1>}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({category:"gems"});
  let Gems = {};

  for (let item of products) {
    if (item.title in Gems) {
      if (
        !Gems[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        Gems[item.title].color.push(item.color);
      }
      if (
        !Gems[item.title].reeds.includes(item.reeds) &&
        item.availableQty > 0
      ) {
        Gems[item.title].reeds.push(item.reeds);
      }
      if (
        !Gems[item.title].threads.includes(item.threads) &&
        item.availableQty > 0
      ) {
        Gems[item.title].threads.push(item.threads);
      }
    } else {
      Gems[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        Gems[item.title].color = [item.color];
        Gems[item.title].reeds = [item.reeds];
        Gems[item.title].threads = [item.threads];
      } else {
        Gems[item.title].color = [];
        Gems[item.title].reeds = [];
        Gems[item.title].threads = [];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(Gems)) }, // will be passed to the page component as props
  };
}
